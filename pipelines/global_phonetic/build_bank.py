...
"""
构建伪风格 Bank v2：
- 使用 Identity 过滤（替代混淆熵）
- 按音素分桶
- 桶内 L24 K-Means 聚类提纯
- 两阶段读取：先 L24 聚类选帧，再按需读 L6/L12/L24，控制内存
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build pseudo bank v2")
    parser.add_argument('--data-dir', default="/root/autodl-tmp/anon_test/checkpoints/trainother500_200spk")
    parser.add_argument('--identity-mask', default=None, help="identity mask HDF5 文件")
...
def pick_mask_path(data_dir, gender, mask_arg):
    if mask_arg:
        return mask_arg
    if not gender:
...
def pick_output_path(data_dir, gender, output):
    if output:
        return output
    if not gender:
        return f"{data_dir}/pseudo_bank_v2.pt"
    return f"{data_dir}/pseudo_bank_v2.gender-{gender}.pt"


def _build_bank(args):
    data_dir = args.data_dir
    meta_path = f"{data_dir}/metadata.json"
    mask_path = pick_mask_path(data_dir, args.gender, args.identity_mask)
    output_path = pick_output_path(data_dir, args.gender, args.output)

    print("=" * 60)
    print(f"构建伪风格 Bank v2 - 性别: {args.gender or 'mix'}")
    ...
    # 2. 加载 identity mask + phones
    print("[2/5] 加载 Identity 掩码 + phones...")
    with h5py.File(mask_path, 'r') as h5:
        keep_mask = h5['keep_mask'][:]
        tau = h5.attrs.get('tau', 'unknown')

    kept_count = int(np.sum(keep_mask))
    print(f"  τ={tau}, 保留帧: {kept_count} / {total_frames} ({kept_count/total_frames:.1%})")
    ...
    print("\n" + "=" * 60)
    print(f"Bank v2 构建完成: {output_path}")
    print(f"  涵盖音素数: {len(pseudo_bank)}")
    print(f"  总帧数: {total_bank_frames}")
    print(f"  格式: {{phone_id: {{'l6': tensor, 'l12': tensor, 'l24': tensor}}}}")
    print("=" * 60)
