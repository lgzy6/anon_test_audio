...
SPLITS_JSON = CKPT_DIR / "pool_splits_20spk.json"
LAYERS      = [6, 24]
SR          = 16000
HOP         = 320
...
def main():
    ...
    parser.add_argument("--pool", type=int, default=None, help="只处理指定 pool")
    parser.add_argument("--out-prefix", default="pool", help="输出目录前缀，如 pool20spk")
    ...
    for pool in pools:
        pid = pool["pool_id"]
        print(f"\n=== Pool {pid} ===")
        pool_dir = CKPT_DIR / "20spk" / f"{args.out_prefix}_{pid}"
