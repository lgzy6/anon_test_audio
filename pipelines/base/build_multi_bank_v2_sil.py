...
def main():
    ...
    parser.add_argument("--frames-per-cluster", "-f", type=int, default=28)
    parser.add_argument("--sub-clusters", "-s", type=int, default=4)
    parser.add_argument("--min-spk-diversity", "-d", type=int, default=2)
    ...
    for pid in pool_ids:
        print(f"\n=== Pool {pid} ===")
        for gender in ["m", "f"]:
            data_dir = CKPT_DIR / "20spk" / f"pool_{pid}" / gender
