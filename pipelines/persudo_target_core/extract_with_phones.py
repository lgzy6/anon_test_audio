#!/usr/bin/env python3
"""优化版特征提取：同时提取 L6/L12/L24，并保存 L6/L12/L24 + phone 标签"""
...
OUTPUT_DIR = "/root/autodl-tmp/anon_test/checkpoints/trainother500_with_phones"
SPEAKER_RATIO = 0.3  # 说话人比例（保留字段，不再用于采样）
UTT_RATIO = 0.5  # 每个说话人保留的 utterance 比例
DEVICE = "cuda"
BATCH_SIZE = 8
FRAMES_PER_SECOND_EST = 100  # 保守估计：10ms hop
SAFETY_FACTOR = 1.15

print("="*60)
print("优化版特征提取 (L6/L12/L24 + Phone 标签)")
print("="*60)
...
# 1. 加载数据集并采样说话人
print("\n[1/4] 加载数据集...")
dataset = LibriSpeechDataset(root=LIBRISPEECH_ROOT, split=LIBRISPEECH_SPLIT)

Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)

print(f"每个说话人保留比例: {UTT_RATIO:.2f}")
free_gb = shutil.disk_usage(OUTPUT_DIR).free / (1024 ** 3)
print(f"当前可用磁盘空间约: {free_gb:.1f} GB")
...
print(f"说话人数: {len(speaker_to_indices)}, 选中样本: {len(selected_indices)}")
...
h5_l24 = h5py.File(f"{OUTPUT_DIR}/layer_24.h5", 'w')
...
ds_l12 = h5_l12.create_dataset('features', shape=(0, 1024), maxshape=(None, 1024),
                               dtype='float32', chunks=(1000, 1024))
ds_l24 = h5_l24.create_dataset('features', shape=(0, 1024), maxshape=(None, 1024),
                               dtype='float32', chunks=(1000, 1024))
...
for idx in tqdm(selected_indices):
    ...
    if len(batch_wavs) >= BATCH_SIZE or idx == selected_indices[-1]:
        ...
        for i, info in enumerate(batch_infos):
            ...
            ds_l24.resize((end, 1024))
            ds_phones.resize((end,))

            ds_l6[start:end] = l6
            ds_l12[start:end] = l12
            ds_l24[start:end] = l24
