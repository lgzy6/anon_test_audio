#pipeline_top3_enhanced.py
"""
基于现有 pipeline 的改进版:
  唯一新增: 静音质心扰动 (sil_centroid_noise)
  在 bank 静音质心上加随机噪声, 非源 L6, 不泄漏身份
  
新增 yaml 参数:
  sil_centroid_noise: 0.1   # 静音质心扰动强度 (0=关闭, 走正常检索)
"""
import sys
import io
import json
...
class PseudoTargetPipelineTop3:
    def __init__(self, config, force_compute=False, devices=None):
        ...
        self.alpha_m = config['modules'].get('alpha_m', self.alpha)
        self.alpha_f = config['modules'].get('alpha_f', self.alpha)
        ...
        # ── 新增: 静音质心扰动 ──
        self.sil_centroid_noise = config['modules'].get('sil_centroid_noise', 0.0)
    ...
    def _load_models(self):
        ...
        print(f"[Enhanced] 模型加载完成 (mode={self.mode}, "
              f"alpha_m={self.alpha_m}, alpha_f={self.alpha_f}, "
              f"top_k={self.top_k}, pools={self.n_pools}, "
              f"sil_noise={self.sil_centroid_noise}, "
    ...
    def _process_dataset(self, dataset_name, dataset_path):
        ...
        for i in tqdm(range(0, len(entries), self.batch_size), desc=f"Enhanced {dataset_name}"):
    ...
    def _anonymize_batch(self, batch, utt2gender):
        ...
        h_lengths = [h_anon_list[j].shape[0] for j in range(len(metas))]
        VOC_BATCH = 8
        with torch.no_grad():
    ...
    def _retrieve(self, src_l6, src_l24, phones, src_gender):
        ...
        if self.use_gender_alpha:
            base_alpha = self.alpha_m if src_gender == 'm' else self.alpha_f
        else:
        ...
        for phone_id in torch.unique(phones_adj):
            ...
            # ── 静音帧: bank 质心 + 随机扰动 ──
            if ph in {0, 1} and self.sil_centroid_noise > 0:
                entry = bank.get(ph, None)
                if entry is not None and entry['l6'].shape[0] > 0:
                    sil_l6 = entry['l6']
                    idx_sel = torch.randint(0, sil_l6.shape[0], (N_q,), device=self.device)
                    base = sil_l6[idx_sel]
                    noise = torch.randn_like(base) * self.sil_centroid_noise
                    h_anon[mask] = base + noise
                else:
                    h_anon[mask] = fallback['l6'].mean(0).expand(N_q, -1)
                continue
