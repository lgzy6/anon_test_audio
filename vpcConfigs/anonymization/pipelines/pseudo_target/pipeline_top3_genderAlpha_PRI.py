"""pipeline_top3_genderAlpha_PRI.py
性别自适应 Alpha + Prosody Residual Injection (PRI) 版本

核心创新:
  1. 性别自适应 Alpha: 男女 cliff 点不同，分别设置最优 alpha
  2. PRI (Prosody Residual Injection): 检索后注入源帧的韵律偏差
     h_final[t] = h_anon[t] + γ · (src_l6[t] - phone_mean_l6)
     - 残差是零均值的，不包含说话人身份的绝对频谱位置
     - 残差编码的是帧级韵律动态（能量变化、频谱倾斜变化）
     - γ 控制韵律保留强度，与 α 正交

原理:
  质心替换抹掉了帧级变化 → SER 无法区分 SAD/ANG
  残差 = src_l6[t] - mean(src_l6 for same phone)
       = 该帧相对于音素均值的偏差
       = 韵律动态信号（能量起伏、频谱倾斜方向）
  残差不包含身份：身份编码在绝对位置（被 phone_mean 吸收）
  残差是零均值的：sum(residual) = 0，ASV 难以利用
"""
import sys
import io
import json
...
class PseudoTargetPipelineTop3GenderAlphaPRI:
    def __init__(self, config, force_compute=False, devices=None):
        ...
        self.prosody_gamma = config['modules'].get('prosody_gamma', 0.0)
        # γ=0.0: 关闭 PRI（等价于原始检索）
        # γ=0.1~0.5: 注入韵律偏差强度
    ...
    def _load_models(self):
        ...
        # 打印配置信息
        alpha_str = f"gender_alpha=(m:{self.alpha_m}, f:{self.alpha_f})" if self.use_gender_alpha else f"alpha={self.alpha}"
        print(f"[Top3-PRI] 模型加载完成 (mode={self.mode}, {alpha_str}, "
              f"top_k={self.top_k}, pools={N_POOLS}, "
              f"prosody_gamma={self.prosody_gamma}, "
              f"bank={self.banks_dir.name})")
    ...
    def _process_dataset(self, dataset_name, dataset_path):
        ...
        for i in tqdm(range(0, len(entries), self.batch_size), desc=f"Top3-PRI {dataset_name}"):
    ...
    def _anonymize_batch(self, batch, utt2gender):
        ...
        for j, (utt_id, src_gender, out_path) in enumerate(metas):
            ...
            h_retrieved = self._retrieve(
                l6_batch[j, :T], l24_batch[j, :T], phones_batch[j], src_gender)
            h_anon_list.append(h_retrieved)
            h_lengths.append(h_retrieved.shape[0])

        VOC_BATCH = 8
        with torch.no_grad():
            for start in range(0, len(metas), VOC_BATCH):
                end = min(start + VOC_BATCH, len(metas))
                sub_h = h_anon_list[start:end]
                max_T = max(h.shape[0] for h in sub_h)
                h_padded = torch.zeros(end - start, max_T, 1024, device=self.device)
                for k, h in enumerate(sub_h):
                    h_padded[k, :h.shape[0]] = h
                wav_batch_out = self.vocoder(h_padded)
                for k in range(end - start):
                    out_len = h_lengths[start + k] * 320
                    wav_out = wav_batch_out[k, :out_len]
                    utt_id, src_gender, out_path = metas[start + k]
    ...
    def _retrieve(self, src_l6, src_l24, phones, src_gender):
        ...
        # 
        # 原理:
        #   对每个音素 p，计算源帧在该音素内的均值 μ_p
        #   残差 r[t] = src_l6[t] - μ_p  （帧相对于音素均值的偏差）
        #   注入: h_final[t] = h_anon[t] + γ · r[t]
        #
        # 残差编码的信息:
        #   - 帧间能量变化（SAD 的下降趋势、ANG 的高波动）
        #   - 频谱倾斜动态（情感相关的音色变化）
        #   - 时序微结构（不同帧的相对位置关系）
        #
        # 残差不包含的信息:
        #   - 说话人身份（编码在 μ_p 中，被减掉了）
        #   - 残差是零均值的: Σ r[t] = 0，ASV 难以利用
        # ══════════════════════════════════════════════════════════
        if self.prosody_gamma > 0:
            for phone_id in torch.unique(phones_adj):
                ph = int(phone_id.item())
                mask = phones_adj == phone_id
                ...
                # 该音素所有源帧的均值（身份基线）
                phone_mean = l6_adj[mask].mean(dim=0)  # [1024]

                # 帧级韵律残差（零均值）
                residual = l6_adj[mask] - phone_mean    # [N, 1024]

                # 注入到匿名输出
                h_anon[mask] = h_anon[mask] + self.prosody_gamma * residual
