"""pipeline_top3Random_unitQuery_20pool_dynAlpha.py — 动态alpha：每个音素在 [alpha_low, alpha_high] 间均匀随机采样"""
import sys
import io
import json
import random
...
class PseudoTargetPipelineTop3:
    def __init__(self, config, force_compute=False, devices=None):
        ...
        # SPN 重定向参数
        self.spn_redirect_threshold = config['modules'].get('spn_redirect_threshold', None)
        # None = 自动用中位数; 正数 = 固定阈值
        self.spn_redirect_alpha = config['modules'].get('spn_redirect_alpha', 0.0)
        # 重定向后用的 alpha，建议 0.0（纯 L24，保语义不泄露身份）
