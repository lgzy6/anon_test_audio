...
# 静音音素集合: 跳过检索，直接输出零向量(vocoder会合成静默)
SILENCE_PHONES = {0, 1}
...
class PseudoTargetPipelineTop3:
    def __init__(self, config, force_compute=False, devices=None):
        ...
        self.silence_noise_sigma = config['modules'].get('silence_noise_sigma', 0.0)
    ...
    def _load_models(self):
        ...
        print(f"[Top3] 模型加载完成 (mode={self.mode}, alpha={self.alpha}, top_k={self.top_k}, "
              f"pools={N_POOLS}, adaptive_alpha={self.use_adaptive_alpha}, "
    ...
    def _anonymize_batch(self, batch, utt2gender):
        ...
        with torch.no_grad():
            for start in range(0, len(metas), VOC_BATCH):
                ...
                wav_batch_out = self.vocoder(h_padded)  # [B, L_out]
