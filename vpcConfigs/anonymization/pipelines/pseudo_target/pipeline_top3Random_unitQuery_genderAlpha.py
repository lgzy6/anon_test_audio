"""pipeline_top3Random_unitQuery_genderAlpha.py
性别自适应 Alpha 版本
原理: 男女声学的 cliff 点不同，分别设置最优 alpha
  - 男性: cliff 在 0.2-0.3 → alpha_m=0.2 保安全
  - 女性: cliff 在 0.3-0.4 → alpha_f=0.3 拿效用
"""
import sys
import io
import json
...
class PseudoTargetPipelineTop3GenderAlpha:
    def __init__(self, config, force_compute=False, devices=None):
        ...
        if self.use_gender_alpha:
            self.alpha_m = config['modules'].get('alpha_m', 0.2)  # 男性默认 0.2
            self.alpha_f = config['modules'].get('alpha_f', 0.3)  # 女性默认 0.3
            self.alpha = None  # 不使用全局 alpha
            print(f"[Top3-GenderAlpha] 启用性别自适应 Alpha: alpha_m={self.alpha_m}, alpha_f={self.alpha_f}")
        else:
            self.alpha = config['modules'].get('alpha', 0.2)
            self.alpha_m = None
            self.alpha_f = None
            print(f"[Top3-GenderAlpha] 使用全局 Alpha: alpha={self.alpha}")
    ...
    def _load_models(self):
        ...
        print(f"[Top3-GenderAlpha] 模型加载完成 ({mode_str})")
    ...
    def _process_dataset(self, dataset_name, dataset_path):
        ...
        for i in tqdm(range(0, len(entries), self.batch_size), desc=f"Top3-GenderAlpha {dataset_name}"):
