"""pipeline_top3_twoStage.py
两阶段检索策略:
  Stage 1: 用 alpha 混合距离选 top-screen_k 候选 (隐私筛选)
  Stage 2: 在候选中按 L6 距离重排选 top_k, 随机取 1 (效用保真)
"""
import sys
import io
import json
...
class PseudoTargetPipelineTop3:
    def __init__(self, config, force_compute=False, devices=None):
        ...
        self.screen_k = config['modules'].get('screen_k', 15)
    ...
    def _load_models(self):
        ...
        print(f"[TwoStage] 模型加载完成 (mode={self.mode}, alpha={self.alpha}, "
              f"screen_k={self.screen_k}, top_k={self.top_k}, pools={N_POOLS}, "
              f"adaptive_alpha={self.use_adaptive_alpha}, bank={self.banks_dir.name})")
    ...
    def _retrieve(self, src_l6, src_l24, phones, src_gender):
        ...
        for phone_id in torch.unique(phones_adj):
            ...
            # ── 两阶段检索 ──
            # Stage 1: 混合距离筛选 top-screen_k (隐私筛选)
            K_screen = min(self.screen_k, N_t)
            screen_idx = dist_mix.topk(K_screen, dim=-1, largest=False).indices  # [N_q, K_screen]

            # Stage 2: 在候选中按 L6 距离重排选 top_k (效用保真)
            screen_l6_dist = dist_l6.gather(1, screen_idx)  # [N_q, K_screen]
            k_final = min(self.top_k, K_screen)
            rerank_idx = screen_l6_dist.topk(k_final, dim=-1, largest=False).indices  # [N_q, k_final]
            final_idx = screen_idx.gather(1, rerank_idx)  # [N_q, k_final]

            rand_col = torch.randint(0, k_final, (N_q,), device=self.device)
            selected_idx = final_idx[torch.arange(N_q, device=self.device), rand_col]
