"""pipeline_top3Random_unitQuery_20pool_spnRedirect_genderAlpha_sadEnergy.py

在 spnRedirect_genderAlpha 版本基础上的第三处增量改动：
  ...
  改动 3 —— SAD 能量包络旁路（波形域后补偿，专精 SAD）
    ...
    sad 门控（可选）：仅在句级 P(sad) 超阈值时施加，缩小暴露面 +
...
class PseudoTargetPipelineTop3:
    def __init__(self, config, force_compute=False, devices=None):
        ...
        if sad_cfg and sad_cfg.get('enable', False):
            ...
            from prosody_sad_energy import SadEnergyBypass
            ...
            # use_gate=True  → 句级 sad 门控（EER 活下来后再开）
    ...
    def _load_models(self):
        ...
        if self.sad_energy is not None:
            sad_desc = (f"on(beta={self.sad_energy.beta}, "
                        ...
                        f"gate={'sad' if self.emotion_gate else 'all-utt'})")

        print(
            f"[spnRedirect+genderAlpha+sadEnergy] 模型加载完成 "
    ...
    # ── 改动 3：句级 sad 概率（复用已算的源特征，不再跑一遍模型） ──
    @torch.no_grad()
    def _sad_prob(self, src_l24_TxD):
        """src_l24_TxD: 该句源 L24 特征 [T,1024]。均值池化 → 情感头 → P(sad)。"""
        if self.emotion_head is None:
            return 1.0
        feat = src_l24_TxD.mean(0, keepdim=True)        # [1,1024]
        logits = self.emotion_head(feat)                # [1,4]
        return torch.softmax(logits, dim=-1).squeeze(0)[SAD_IDX].item()
    ...
    def _anonymize_batch(self, batch, utt2gender):
        ...
        # 第一轮：检索 h_anon + （门控开启时）顺手算句级 sad_prob
        h_anon_list = []
        sad_prob_list = []
        for j, (utt_id, src_gender, out_path) in enumerate(metas):
            ...
            if self.sad_energy is not None and self.emotion_gate:
                sad_prob_list.append(self._sad_prob(l24_batch[j, :T]))
            else:
                sad_prob_list.append(1.0)   # 不门控 = 全句施加
        ...
        for j, (utt_id, src_gender, out_path) in enumerate(metas):
            ...
            if self.sad_energy is not None:
                ...
                wav_out = self.sad_energy.process(
                    wav_out, src_wav, sr=16000, sad_prob=sad_prob_list[j])
    ...
    def _anonymize_waveform(self, waveform, output_path, src_gender='m'):
        ...
        if self.sad_energy is not None:
            ...
            sad_prob = (self._sad_prob(l24)
                        if self.emotion_gate else 1.0)
            wav = self.sad_energy.process(
                wav, src_wav, sr=16000, sad_prob=sad_prob)
