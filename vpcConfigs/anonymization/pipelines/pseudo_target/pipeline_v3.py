"""VPC 2024 Pipeline — kNN 检索匿名化 (V3)"""
import sys
import io
import json
import random
...
class PseudoTargetPipelineV3:
    def __init__(self, config, force_compute=False, devices=None):
        ...
        self.alpha   = config['modules'].get('alpha', 0)
    ...
    def _load_models(self):
        ...
        print(f"[V3] 模型加载完成 (mode={self.mode}, alpha={self.alpha}, top_k={self.top_k}, pools={N_POOLS})")
            ...
            for line in utt2spk_file.read_text().strip().split('\n'):
                ...
                if len(parts) == 2:
                    ...
                    if spk not in spk2gender:
                        print(f"[Warning] Speaker {spk} not in spk2gender, defaulting to 'm'")
            ...
            try:
                if ref_type == 'pipe':
                    ...
                    if proc.returncode != 0:
                        print(f"[Warning] Pipe command failed for {utt_id}")
                        continue
    ...
    def _retrieve(self, src_l6, src_l24, phones, src_gender):
        ...
        for phone_id in torch.unique(phones_adj):
            ...
            dist_mix = self.alpha * dist_l6 + (1 - self.alpha) * dist_l24
